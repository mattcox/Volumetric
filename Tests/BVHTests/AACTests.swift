...
@Test
func aacProducesValidTree() throws {
	var generator = LCG(seed: 7)
	let elements = makeBounds(200, using: &generator, as: Vector3<Float>.self)
	let tree = AAC(maximumLeafSize: 4).build(elements, bounds: Bounds(elements)!)
	validate(tree, count: elements.count, maximumLeafSize: 4)
}

@Test
func aacFastProducesValidTree() throws {
	var generator = LCG(seed: 8)
	let elements = makeBounds(200, using: &generator, as: Vector3<Float>.self)
	let tree = AAC(delta: 4, epsilon: 0.2, maximumLeafSize: 4).build(elements, bounds: Bounds(elements)!)
	validate(tree, count: elements.count, maximumLeafSize: 4)
}

@Test
func aacProducesValidTreeIn2D() throws {
...
@Test
func aacProducesValidTreeIn4D() throws {
...
@Test
func aacSingleElementIsALeafRoot() throws {
	let elements = [Bounds(min: V(0, 0, 0), max: V(1, 1, 1))]
	let tree = AAC().build(elements, bounds: Bounds(elements)!)
...
@Test
func aacTerminatesOnCoincidentCentroids() throws {
...
@Test
func aacHandlesCoplanarData() throws {
...
@Test
func aacWithSingletonLeaves() throws {
	var generator = LCG(seed: 19)
	let elements = makeBounds(100, using: &generator, as: Vector3<Float>.self)
	let tree = AAC(maximumLeafSize: 1).build(elements, bounds: Bounds(elements)!)
...
@Test
func aacIsDeterministic() throws {
	var generator = LCG(seed: 23)
	let elements = makeBounds(200, using: &generator, as: Vector3<Float>.self)
	let bounds = Bounds(elements)!

	let first = AAC(maximumLeafSize: 4).build(elements, bounds: bounds)
	let second = AAC(maximumLeafSize: 4).build(elements, bounds: bounds)
...
@Test
func aacIsCheaperToTraverseThanMedianSplit() throws {
	var generator = LCG(seed: 5)
	let elements = makeBounds(500, using: &generator, as: Vector3<Float>.self)
	let bounds = Bounds(elements)!

	let aac = sahCost(AAC(maximumLeafSize: 4).build(elements, bounds: bounds))
...
@Test
func aacClosestMatchesBruteForce() throws {
	var generator = LCG(seed: 3)
	let boxes = makeBoxes(200, using: &generator)
	let bvh = try #require(BVH(boxes, using: .aac(maximumLeafSize: 4)))
...
@Test
func aacRayHitMatchesBruteForce() throws {
